...
class Crate(
    ...
    init {
        ...
        try {
            ...
            throw MojoExecutionException(
                "Failed to parse Cargo.toml file: " + e.message
            )
        }
    ...
    private val dirName: String
        get() = crateRoot.getFileName().toString()

    private val profile: String
        get() = if (params.release) "release" else "debug"

    fun hasCdylib(): Boolean {
        try {
    ...
    private val binNames: List<String?>
        get() {
            ...
            for (index in 0 until bins.size()) {
                val bin: TomlTable = bins.getTable(index)
                    ?: throw MojoExecutionException(
                        "Failed to extract `bin`s from Cargo.toml file: " +
    ...
    private fun runCommand(args: List<String?>) {
        ...
        Executors.newSingleThreadExecutor().submit(Runnable {
            BufferedReader(InputStreamReader(process.inputStream))
                .lines()
                .forEach(Consumer<String> { charSequence: String? -> log.info(charSequence) })
        })
    ...
    private fun cargo(args: List<String?>) {
        ...
        if (!params.environmentVariables.isEmpty()) {
            ...
            for (key in params.environmentVariables.keys) {
                log.info(
                    "  $key=" + params.environmentVariables[key]?.let {
                        Shlex.quote(
                            it
                        )
                    }
    ...
    fun copyArtifacts() {
        ...
        for (artifactPath in artifactPaths) {
            var fileName: Path = if (params.copyWithPlatformDir) artifactPath.getFileName() else
                Paths.get(JniLibLoader.getJniLibPath(JniLibLoader.getLibName(artifactPath.getFileName().toString()), false))
            val destPath: Path = copyToDir.resolve(fileName)
//            log.info("fileName:"+fileName.toString()+", destPath:"+destPath.toString())
            try {
                Files.copy(
                    artifactPath,
                    destPath,
