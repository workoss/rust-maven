...
public final class JniLibLoader {
    ...
    public synchronized boolean loadLibrary(
        ...
        boolean isLoadSystem =
                Stream.of(getJniLibNameWithoutSuffix(libName), libName)
                        .anyMatch(
                                lib -> {
                                    try {
    ...
    private static String getJniLibNameWithoutSuffix(String libName) {
        libName = libName.replace("-", "_");
        String libPrefix = OS.isWindows() ? "" : "lib";
        return libPrefix + libName;
    }

    private static String getJniLibName(String libName) {
        libName = libName.replace("-", "_");
        String libPrefix = OS.isWindows() ? "" : "lib";
        String libSuffix = OS.isWindows() ? ".dll" : OS.isOSX() ? ".dylib" : ".so";
